    ...
    <div className="max-w-full mx-auto space-y-6 fade-in">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 flex items-center justify-between flex-wrap gap-4 card-hover slide-up">
        <div className="flex items-center space-x-3">
          <div className="bg-green-100 p-3 rounded-lg shadow pulse-slow">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-lg p-6 card-hover slide-up">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Add New Production Record</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Planned Volume (kg)</label>
              <input
                type="number"
                step="0.01"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Actual Volume (kg)</label>
              <input
                type="number"
                step="0.01"
                value={formData.actualVolume}
                onChange={(e) => setFormData({ ...formData, actualVolume: e.target.value })}
                className="form-input"
                required
              />
            </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 card-hover slide-up">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Production Records</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-100">
                  <th className="px-4 py-3 text-sm font-semibold text-gray-700">Factory</th>
                  <th className="px-4 py-3 text-sm font-semibold text-gray-700">Week</th>
                  <th className="px-4 py-3 text-sm font-semibold text-gray-700">Year</th>
                  <th className="px-4 py-3 text-sm font-semibold text-gray-700">Planned (kg)</th>
                  <th className="px-4 py-3 text-sm font-semibold text-gray-700">Actual (kg)</th>
                </tr>
              </thead>
              <tbody>
                {productionData.map((d, idx) => (
                  <tr
                    key={d.id}
                    className="hover:bg-gray-50 border-b last:border-0 slide-up"
                    style={{ animationDelay: `${idx * 0.05}s` }}
                  >
                    <td className="px-4 py-3 text-gray-800">{factories.find((f) => f.id === d.factoryId)?.name}</td>
                    <td className="px-4 py-3 text-gray-800">{d.week}</td>
                    <td className="px-4 py-3 text-gray-800">{d.year}</td>
                    <td className="px-4 py-3 text-gray-800">{formatNumber(d.plannedVolume)}</td>
                    <td className="px-4 py-3 text-gray-800">{formatNumber(d.actualVolume)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
